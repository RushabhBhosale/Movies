  ...
  const getProgressInfo = (movie: any) => {
    ...
    if (isTV) {
      ...
      return {
        ...
        percent,
        icon: <Tv size={14} className="text-accent-foreground" />,
        showProgress: true,
      };
    } else {
      return {
      ...
      <div className="flex justify-between items-center mb-6 gap-4">
        ...
        >
          <div className="overflow-x-auto no-scrollbar flex">
            <TabsList className="flex w-max bg-zinc-800 rounded-lg p-1">
              {statuses.map((status) => (
                <TabsTrigger
                  key={status.id}
                  value={status.name}
            ...
            <Button
              ...
              variant="outline"
              className="ms-3 hidden md:block"
            >
              {!grid ? <Grid /> : <List />}
            </Button>
          </div>
                        ...
                        <div className="col-span-2 flex flex-col">
                          ...
                          {progressInfo.showProgress && (
                            <div className="w-full bg-zinc-700 rounded-full h-1.5 mt-1">
                              <div
                                className="bg-accent-foreground h-1.5 rounded-full transition-all duration-300"
